...
This section explains how to deploy a MinIO tenant using certificates generated by [cert-manager](https://cert-manager.io/) 
...
The `Cluster Issuer` is created in the default namespace.

Is recommended to create a local `Issuer` in each tenant namespace.

As well create one `Issuer` in the minio-operator namespace, we will explain services that require TLS certificates in the `minio-operator` namespace in the section [MinIO Operator services with cert-manager](# MinIO Operator services with cert-manager) below.
...
Copy the cert-manager generated CA certificate into the minio-operator namespace. This allows Operator to trust the cert-manager issued CA and the derived certificates.
...
MinIO Operator manages the TLS certificate issuing for the services hosted in the `minio-operator` namespace, such services 
are the Secure Token Service `sts` and the Console `console` service.

## Secure Token Service (STS)

MinIO STS is a service included MinIO Operator, build to provide Native IAM Authentication for Kubernetes, in essence,
this allows you to control access to your MinIO tenant from your applications without having to  explicitly create credentials 
for each application, for more information on the Service check on the MinIO docs https://min.io/docs/minio/kubernetes/upstream/developers/sts-for-operator.html
there is also a little [STS](https://github.com/minio/operator/blob/master/docs/STS.md) guide in the docs dir with some 
example client code in https://github.com/minio/operator/tree/master/examples/kustomization/sts-example.

For what matters in this guide, is that the STS Service is a webserver, and as such is presented with a TLS certificate,
this guide covers how to *disable* the automatic generation of the certificate in Operator and issue the certificate using
...
When you install MinIO Operator, by default the Console service comes with TLS disabled, in this guide we will explain 
how to issue the TLS certificate for this service and enable TLS.
...
- `kubectl` access to your `k8s` cluster
- [cert-manager](https://cert-manager.io/docs/releases/release-notes/release-notes-1.12/) 1.12.X (preferred) or later installed.
- Wait to install Operator until certificates are issued by Cert Manager, make sure that step [Create Cluster Self-signed root Issuer ](#Create Cluster Self-signed root Issuer) mentioned above in done. 


### Create minio-operator namespace CA intermediary Issuer

Create the Intermediary Certificate Authority (CA) certificate used to issue certificates for services in the `minio-operator` namespace.
...
```
Next, request a Certificate with `isCA: true` specified. This is our intermediary CA for the `minio-operator` namespace.
...
```
A new secret with the name `operator-ca-tls` will be created in the `minio-operator` namespace, since this is the CA issuing TLS certificates for the services, MinIO Operator will copy and keep synched this secret into the Tenant namespaces with a certificate named the same `operator-ca-tls`.
...
> * `cluster domain` is the DNS assigned in your Kubernetes cluster. Typically this is `cluster.local`, check on your coredns
> configuration for the correct value for your Kubernetes cluster. For example, the output of `kubectl get configmap coredns -n kube-system -oyaml | yq ".data"` can vary depending on the Kubernetes distribution (Openshift, Rancher, EKS, etc.)
...
This will create a secret called `sts-tls` in the `minio-operator` namespace. 


### Create TLS certificate for Console service

Request `cert-manager` issue a new certificate based on the following internal domains for console:
...
This will create a secret called `console-tls` in the `minio-operator` namespace.

### Install Operator with Auto TLS disabled for STS

When installing the Operator deployment, make sure to set `OPERATOR_STS_AUTO_TLS_ENABLED: off` env variable in the `minio-operator` container, this will prevent
MinIO Operator from issuing the certificate for STS, and instead will wait for you to provide the TLS certificate issued by Cert Manager.

Also make sure to set `MINIO_CONSOLE_TLS_ENABLE: off` env variable, this will prevent MinIO Operator from issuing the certificate for Console
and instead will wait for you to provide the TLS certificate issued by Cert Manager.

> [!WARNING]
> Failing to provide the secret containing the TLS certificate or providing an invalid key-pair in the secret will
