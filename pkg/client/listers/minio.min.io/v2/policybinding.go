...
type PolicyBindingLister interface {
	...
	List(selector labels.Selector) (ret []*v2.PolicyBinding, err error)
...
// List lists all PolicyBindings in the indexer.
func (s *policyBindingLister) List(selector labels.Selector) (ret []*v2.PolicyBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.PolicyBinding))
	})
	return ret, err
}
...
type PolicyBindingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.PolicyBinding, error)
...
// List lists all PolicyBindings in the indexer for a given namespace.
func (s policyBindingNamespaceLister) List(selector labels.Selector) (ret []*v2.PolicyBinding, err error) {
...
// Get retrieves the PolicyBinding from the indexer for a given namespace and name.
func (s policyBindingNamespaceLister) Get(name string) (*v2.PolicyBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("policybinding"), name)
	}
	return obj.(*v2.PolicyBinding), nil
}
